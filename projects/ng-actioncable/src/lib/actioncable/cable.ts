import { Consumer, ActionCable, Subscription } from '.'
...
export class Cable {
  ...
  constructor(public url: string, public params?: any) {
    ...
    this.baseCable = this.actionCable.createConsumer(this.buildUrl(url, params))

    // If a function is passed as params, re-evaluate it before attempting to reconnect
    if (params instanceof Function) {
      this.disconnected().subscribe(() => {
        this.baseCable.url = this.actionCable.createWebSocketURL(this.buildUrl(url, params))
      })
    }
  }
